"""DSpark → RWKV · 阶段2v3 真实文本对比实验

与 stage2_train_v3.py 完全一致，仅数据源不同：
- v3 原版：target 自回归生成（stage2_train_5layer.pt）
- 本版：  LAMBADA 真实文本（stage2_train_text.pt）

只跑 block=4（v3 已证明最优配置），与 v3 baseline 直接对比接受率。
"""
import math
import torch
...
DATA_PATH = Path(__file__).parent / "data" / "stage2_train_text.pt"  # 仅此处不同
...
def run(block, n_steps=N_STEPS):
    print(f"\n{'='*60}\n=== RWKV-7 真实文本 (block={block}, 5layer, steps={n_steps}) ===\n{'='*60}")
...
def main():
    print(f"DEVICE={DEVICE} 真实文本(LAMBADA) block=4 5000步")
    pos_rate, avg = run(4)
    print(f"\n{'='*60}\n对比汇总\n{'='*60}")
    print(f"{'数据源':<20} {'平均':<8} 各位置")
    print(f"{'LAMBADA 真实文本':<20} {avg:.4f}   {[f'{r:.3f}' for r in pos_rate]}")
    print(f"{'target 自回归(v3)':<20} 0.9795   ['0.972', '0.992', '0.988', '0.969']")
