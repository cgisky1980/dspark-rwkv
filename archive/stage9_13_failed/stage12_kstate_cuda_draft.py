"""DSpark → RWKV · 阶段12: CUDA 加速 target + draft 的 K-state 投机解码

基于 stage11,但 draft 也换成 stage11_cuda_target.RWKV7Target2p9BCuda(Albatross faster3a)。
target 和 draft 都用 CUDA kernel,消除所有 Python for 循环。
