...
MyModule = nn.Module
MyFunction = __nop  # Windows 上 torch.compile 不稳定，禁用
MyStatic = __nop
MyDisable = __nop
...
############################################### gems ###################################################
# import flag_gems  # Windows 无 triton，禁用。所有 torch.ops.flag_gems.* 已替换为 F.linear 等价实现
...
def RWKV_x070_CMix_one(x, x_prev, x_k, K_, V_):
    ...
    kv = F.linear(k, V_)  # 原: torch.ops.flag_gems.rwkv_mm_sparsity(k, V_)
    return kv
